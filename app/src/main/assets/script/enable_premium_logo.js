(function () {
    if (window.enablePremiumLogo?.syncPreferences) {
        window.enablePremiumLogo.syncPreferences();
        return;
    }

    const PREMIUM_LOGO_SVG = `
        <span class="yt-icon-shape style-scope yt-icon ytSpecIconShapeHost">
            <div style="width: 100%; height: 100%; display: block; fill: currentcolor; padding-left:16px;">
                <svg xmlns="http://www.w3.org/2000/svg" id="yt-ringo2-red-svg_yt8" class="external-icon" width="101" height="20" viewBox="0 0 101 20" focusable="false" aria-hidden="true" style="pointer-events: none; display: inherit; width: 100%; height: 100%;">
                    <g>
                        <path d="M14.4848 20C14.4848 20 23.5695 20 25.8229 19.4C27.0917 19.06 28.0459 18.08 28.3808 16.87C29 14.65 29 9.98 29 9.98C29 9.98 29 5.34 28.3808 3.14C28.0459 1.9 27.0917 0.94 25.8229 0.61C23.5695 0 14.4848 0 14.4848 0C14.4848 0 5.42037 0 3.17711 0.61C1.9286 0.94 0.954148 1.9 0.59888 3.14C0 5.34 0 9.98 0 9.98C0 9.98 0 14.65 0.59888 16.87C0.954148 18.08 1.9286 19.06 3.17711 19.4C5.42037 20 14.4848 20 14.4848 20Z" fill="#FF0033"></path>
                        <path d="M19 10L11.5 5.75V14.25L19 10Z" fill="white"></path>
                    </g>
                    <g id="youtube-paths_yt81">
    ...
    function readEnabled() {
        try {
            return !!JSON.parse(lite.getPreferences() || "{}").enable_premium_logo;
        } catch {
            return false;
        }
    }
    ...
    function getTrustedHTML(html) {
        if (window.trustedTypes && window.trustedTypes.createPolicy) {
            try {
                const policy = window.trustedTypes.createPolicy('lite-premium-logo-policy', {
    ...
    function getLogoTargets() {
        const targets = [];
        document.querySelectorAll('yt-icon#logo-icon').forEach(el => targets.push(el));
        if (location.hostname === "m.youtube.com") {
            document.querySelectorAll('ytm-home-logo yt-icon, ytm-pivot-bar-renderer yt-icon').forEach(el => {
                const svg = el.querySelector('svg');
                if (svg && !targets.includes(el)) targets.push(el);
            });
            if (targets.length === 0) {
                const homeLogo = document.querySelector('ytm-home-logo');
                if (homeLogo) targets.push(homeLogo);
            }
        }
        return targets;
    }
    ...
    function updateLogo() {
        if (!enabled) return;
        getLogoTargets().forEach(icon => {
            if (!(icon instanceof HTMLElement)) return;
            if (icon.innerHTML.includes('yt-ringo2-red-svg_yt8')) return;
            ...
            if (icon.innerHTML.toLowerCase().includes('premium')) return;
            icon.innerHTML = getTrustedHTML(PREMIUM_LOGO_SVG);
        });
    }
